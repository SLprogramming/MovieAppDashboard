...
const AdminChatUI = () => {
  ...
  const displayedMessage = useMemo(() => {
    return messages.filter(
      (msg) => msg.conversation_id.toString() === selectedChatId,
    );
  }, [messages, selectedChatId]);

  const selectedCustomer = useMemo(() => {
    return conversations.find((convo) => convo._id === selectedChatId)
      ?.request_user_id;
  ...
  useEffect(() => {
    ...
    messageSocket.on("pendingConversation:new", (data: any) => {
  ...
  const takeConversation = (id: string) => {
    ...
    // dispatch(fetchMessagesByConversationId(id));
  ...
  return (
    <div className="flex h-[90vh] w-full bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm outline-none focus:border-black"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {newConversation.length > 0 && (
            <p className="text-center text-gray-500 text-1xl font-bold">
              New Conversation
            </p>
          )}
          ...
          {newConversation.length > 0 && (
            <div className="w-full border border-bray-50" />
          )}
      ...
      <div className="flex-1 flex flex-col bg-white">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <h3 className="font-bold text-gray-800">
              {selectedCustomer?.name || "Username"}
            </h3>
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/20 scrollbar-hide"
        >
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <div
                    className={`flex items-center gap-1 mt-1 ${msg?.sender_id === userId ? "justify-end" : "justify-start"}`}
                  >
                    <span className="text-[10px] text-gray-400">
        ...
        <div className="p-4 border-t border-gray-100">
          <form
            ...
            className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 focus-within:border-black transition-all"
          >
            <input
              type="file"
              ref={fileInputRef}
            ...
            <button
              ...
              className={`hover:text-black ${selectedFile ? "text-black" : "text-gray-400"}`}
            >
              <Paperclip size={20} />
            </button>
            <input
